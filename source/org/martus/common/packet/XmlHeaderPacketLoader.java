...
public class XmlHeaderPacketLoader extends SimpleXmlDefaultLoader
{
	public XmlHeaderPacketLoader(BulletinHeaderPacket bhpToFill)
	{
		super(bhpToFill.getPacketRootElementName());
		bhp = bhpToFill;
	}
		...
		throws SAXParseException
	{
		...
		String value = null;
		if(getTagsContainingStrings().contains(tag))
			value = ((SimpleXmlStringLoader)ended).getText();
			
		try
		{
			if(tag.equals(MartusXml.PacketIdElementName))
				bhp.setPacketId(value);
			else if(tag.equals(MartusXml.AccountElementName))
				bhp.setAccountId(value);
			else if(tag.equals(MartusXml.BulletinStatusElementName))
				bhp.setStatus(value);
