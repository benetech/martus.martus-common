...
public class TestDammCheckDigitAlgorithm  extends TestCaseEnhanced
{
	...
	public void testBasics()
    {
		...
		assertEquals("Since the default token length is 7 + checkdigit, this should return -1", "-1", checkDigit);
		testDAMMCheckAlgorithm.setMaxNumberDataDigits(3);
		...
		testDAMMCheckAlgorithm.setMaxNumberDataDigits(-8);
		checkDigit = testDAMMCheckAlgorithm.getCheckDigit(tokenData);
		assertEquals("setting the maxNumber of DataDigits negative should still keep the old valud and therefore the check digit should be 4", "4", checkDigit);
		
		tokenData = "3841590";
		testDAMMCheckAlgorithm.setMaxNumberDataDigits(7);
		checkDigit = testDAMMCheckAlgorithm.getCheckDigit(tokenData);
		assertEquals("For data 3841590 the check digit should be 0", "0", checkDigit);
		String validCompleteToken = tokenData + checkDigit;
		assertTrue("For complete token data 38415900 should be true", testDAMMCheckAlgorithm.validateToken(validCompleteToken));
		assertFalse("For a incomplete token data 3841590 should be false", testDAMMCheckAlgorithm.validateToken(tokenData));
	...
	{
		DammCheckDigitAlgorithm accountTokenGenerator = new DammCheckDigitAlgorithm();
